...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 29 21:15:23 2020
...
print(len(slice_num))
data_recon_1 = np.zeros((len(slice_num),data_noisy.shape[1],data_noisy.shape[2]))
...
for slice in slice_num:
    ...
    data_recon_1[slice,:,:] = (img_rec)
print(data_recon_1.shape)    

save2img(data_recon_1[0,:,:], './projProj/proj.png')    

print('loading prediction models in sinogram domain')
 
generator = tf.keras.models.load_model('./projection/model/%s.h5'%(args.sino_modelName), )

print('Predicting in projection domain')  
slice_num = range(data_noisy.shape[0])
print(len(slice_num))
data_recon_2 = np.zeros((len(slice_num),data_noisy.shape[1],data_noisy.shape[2]))
...
for slice in slice_num:
    ...
    #print('Slice number %d'%slice)
    img_rec = img_rec.reshape((data_noisy.shape[1],data_noisy.shape[2]))
    data_recon_2[slice,:,:] = (img_rec)
print(data_recon_2.shape)    
...
arrays = []
print('Combination')
for i in range(data_recon_1.shape[0]):
    arrays.append( data_noisy[i])
    arrays.append( data_recon_1[i])
    arrays.append( data_recon_2[i])
arrays = np.asarray(arrays, dtype=np.float32)
print(arrays.shape)

#
print('Saving noisy dataset into projProj/dataset')
hf = h5py.File('./projProj/dataset/noisy_train_%d.h5'%(args.dose), 'w')
...
print('Saving clean dataset into projProj/dataset')

hf = h5py.File('./projProj/dataset/clean_train.h5', 'w')
hf.create_dataset('images', data = data)
hf.close()

save2img(arrays[0,:,:], './projProj/noisy_1.png')
save2img(arrays[1,:,:], './projProj/noisy_2.png')
save2img(arrays[2,:,:], './projProj/noisy_3.png')

save2img(data[0,:,:], './projProj/clean.png')



print('Saving test noisy dataset into projProj/dataset')
arrays_test = arrays[0:6]
hf = h5py.File('./projProj/dataset/noisy_test_%d.h5'%(args.dose), 'w')
hf.create_dataset('images', data = arrays_test)
hf.close()

data_test = data[0:2]

hf = h5py.File('./projProj/dataset/clean_test.h5', 'w')
