...
def gen_train_batch_bg(x_fn, y_fn, mb_size, in_depth, img_size):
    ...
    print(Y.shape)
    while True:
        idx = np.random.randint(0, (X.shape[0])//in_depth-1, mb_size)
        if X.shape[1] == img_size:
            crop_idx = 0
        else:
            crop_idx = np.random.randint(0, X.shape[1]-img_size)
        
        batch_X = np.array([np.transpose(X[s_idx*in_depth : (s_idx*in_depth+in_depth)], (1, 2, 0)) for s_idx in idx])
        batch_X = batch_X[:, crop_idx:(crop_idx+img_size), crop_idx:(crop_idx+img_size), :]

        batch_Y = np.expand_dims([Y[s_idx] for s_idx in idx], 3)
