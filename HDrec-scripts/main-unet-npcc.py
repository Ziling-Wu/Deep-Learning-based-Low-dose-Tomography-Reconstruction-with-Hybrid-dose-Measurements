...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 27 15:48:09 2020
...
from models import newUnet as make_generator_model           # import a generator model
...
mb_data_iter_val = bkgdGen(data_generator=gen_train_batch_bg(x_fn=args.xtest, \
...
for epoch in range(40001):
    ...
    loss_mse = tf.losses.mean_squared_error(Y_pred, Y_val)
        ...
        # gen_loss = lambda_adv * loss_adv + lambda_mse * loss_mse + lambda_perc * perc_loss + \
        #         lambda_npcc * loss_npcc
    val_error = lambda_mse * loss_mse
    loss_val.append(val_error)
    print('Validation error %.2f'%(val_error))

    if epoch % (200//gene_iters) == 0:
        X222_p, y222 = get1batch4test(x_fn=args.xtest, y_fn=args.ytest, in_depth=in_depth)
        print(X222_p.shape)
        X222 = X222_p[0,0:2112,:,0]
        
        print(X222.shape)
        X222 = X222.reshape((1,X222.shape[0],X222.shape[1],1))
        print(X222.shape)
        print(X222.dtype)
        pred_img = generator.predict(X222)
