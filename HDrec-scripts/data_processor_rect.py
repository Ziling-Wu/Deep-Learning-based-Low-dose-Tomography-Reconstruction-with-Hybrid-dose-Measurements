...
def gen_train_batch_bg(x_fn, y_fn, mb_size, in_depth, img_size):
   ...
   # Y = Y[100:1000,:,:]
    ...
    while True:
        ...
        crop_idx = np.random.randint(0, X.shape[2]-img_size)
        
        batch_X = np.array([np.transpose(X[s_idx : (s_idx+in_depth)], (1, 2, 0)) for s_idx in idx])
        batch_X = batch_X[:, 0:(0+img_size), crop_idx:(crop_idx+img_size), :]

        batch_Y = np.expand_dims([Y[s_idx+in_depth//2] for s_idx in idx], 3)
        batch_Y = batch_Y[:, 0:(0+img_size), crop_idx:(crop_idx+img_size), :]
        #print(batch_X.shape)
        #print(batch_Y.shape)
        yield batch_X, batch_Y
        
        
def gen_train_batch_bg_random(x_fn, y_fn, mb_size, in_depth, img_size):
    X, Y = None, None
    with h5py.File(x_fn, 'r') as hdf_fd:
        X = hdf_fd['images'][:].astype(np.float32)
    #X = X[100:1000,:,:]
